...
function RegisterPageContent() {
  ...
  const createProfileAfterGoogleAuth = async (formDataToUse?: typeof formData) => {
    ...
    try {
      ...
      // Extract and upload Google profile picture URL
      const googleAvatar = extractGoogleAvatar(currentUser)
      console.log('📸 Google avatar URL:', googleAvatar)

      let avatarUrl = null
      if (googleAvatar) {
        try {
          console.log('📤 Uploading Google profile picture to Cloudinary...')
          avatarUrl = await uploadImageFromUrlToCloudinary(googleAvatar)
          console.log('✅ Google profile picture uploaded to Cloudinary:', avatarUrl)
        } catch (uploadError) {
          console.error('❌ Failed to upload Google profile picture to Cloudinary:', uploadError)
          // Fall back to using the Google URL directly if upload fails
          avatarUrl = googleAvatar
          console.log('📸 Using Google URL as fallback')
        }
      }
      ...
      const userData = {
        ...
        avatar: avatarUrl, // text (nullable) - Google profile picture (uploaded to Cloudinary)
